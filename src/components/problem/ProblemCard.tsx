      ...
      <tbody className="text-white  ">
        {isLoading ? (
          ...
          data.result.map((problem: ProblemSchema, idx: number) => {
            ...
            return (
              <tr
                className={`hover:bg-neutral-90 b-[2px] border-b border-neutral-90  cursor-pointer transition-all   `}
                key={idx}
              >
                <Link
                ...
                <td className="px-2 py-4 font-medium whitespace-nowrap ">
                  {problem.isSolved ? (
                    <IonIcon
                      ...
                      className="flex items-center justify-center h-6 w-6 ml-4 text-dark-green-s "
                    />
                  ) : (
                    <Link
                      to={`/problem/${problem.id}`}
                    ...
                    >
                      <IonIcon
                        icon={codeSlashSharp}
                        className=" h-6 w-6 ml-4 text-primary-60"
                      />
                    </Link>
                  )}
                </td>
